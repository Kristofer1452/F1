...
def time_to_sec(time_str):
    # Parse time string in format 'HH:MM:SS.sss'
    dt = datetime.datetime.strptime(time_str, '%H:%M:%S.%f')
    # Convert to timedelta
    time_delta = dt - datetime.datetime(1900, 1, 1)
    # Get total seconds
...
if __name__ == '__main__':
    Filename = 'Result.txt'
    results = []
    fastest_lap = ['Unknown', float('inf')]
    three_sektors = [['Unknown', float('inf')], ['Unknown', float('inf')], ['Unknown', float('inf')]]
    lap_times = {}
    lap_errors = {}
    ...
    with open(Filename, 'r', encoding='utf-8') as file:
        ...
        for line in lines[1:]:
            ...
            lap = int(data[0])
            name = data[1]
            lap_time = time_to_sec(data[2])  # Convert lap time string to seconds
            sector1 = time_to_sec(data[3])  # Convert sector1 time string to seconds
            sector2 = time_to_sec(data[4])  # Convert sector2 time string to seconds
            sector3 = time_to_sec(data[5])  # Convert sector3 time string to seconds
